

public class Main {

    private static TreeMap treeMap1 = new TreeMap();

    public static void main(String[] args) {

        TreeMap treeMap = new TreeMap();
/*
        13 8 1 6 11 17 15 25 22 27
        true
        false
        15 8 1 6 11 25 17 22 27
        11
        true
*/
        treeMap.put(13, 1);
        treeMap.put(8, 11);
        treeMap.put(17, 1);
        treeMap.put(15, 1);
        treeMap.put(25, 1);
        treeMap.put(22, 1);
        treeMap.put(27, 1);
        treeMap.put(11, 1);
        treeMap.put(1, 1);
        treeMap.put(6, 1);
        treeMap.preOrder(treeMap.root);
        System.out.println();
        System.out.println(treeMap.containsKey(1));
        System.out.println(treeMap.isEmpty());
        treeMap.remove(13);
        treeMap.preOrder(treeMap.root);
        System.out.println();
        System.out.println(treeMap.get(8));
        treeMap.clear();
        treeMap.preOrder(treeMap.root);
        System.out.println(treeMap.isEmpty());


        System.out.println();
        System.out.println("Время доавбления 1_000_000 эл-ов:");
        long start = System.currentTimeMillis();
        for (int i = 0; i < 1_000_000; i++) {
            treeMap1.put(i, i + 1);
        }
        long finish = System.currentTimeMillis();
        long timeConsumedMillis = finish - start;
        System.out.println(timeConsumedMillis + " милисекунд(ы)");
        System.out.println(HeightTest(1_000_000));
        System.out.println(TreeMap.getHight(treeMap1.root) + " <= " + 2 * (Math.log10(1_000_001) / Math.log10(2)));
        System.out.println("Коэффициент С равен: " + (TreeMap.getHight(treeMap1.root) / ((Math.log10(1_000_001) / Math.log10(2)))));


        System.out.println("Время удаления 50_000 эл-ов:");
        long start1 = System.currentTimeMillis();
        for (int i = 100_000; i > 50_000; i--) {
            treeMap1.remove(i);
        }
        long finish1 = System.currentTimeMillis();
        long timeConsumedMillis1 = finish1 - start1;
        System.out.println(timeConsumedMillis1 + " милисекунд(ы)");

        System.out.println(HeightTest(50_000));
        System.out.println(TreeMap.getHight(treeMap1.root) + " <= " + 2 * (Math.log(50_001) / Math.log10(2)));

    }

    private static String HeightTest(int k) {
        return (double) TreeMap.getHight(treeMap1.root) <= 2
                * (Math.log(k + 1) / Math.log10(2)) ? "passed" : "failed";
    }

}
